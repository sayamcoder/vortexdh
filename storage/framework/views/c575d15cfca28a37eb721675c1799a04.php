...
<?php $__env->startSection('title', 'Deploy Instance'); ?>
...
<div class="max-w-3xl mx-auto animate-fade">
    ...
    <form action="<?php echo e(route('servers.store')); ?>" method="POST" class="bg-zinc-900 border border-white/5 rounded-xl p-6 md:p-8 shadow-sm">
        <?php echo csrf_field(); ?>
        ...
        <div class="space-y-8">
            ...
            <div class="group">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-zinc-500">Max: <?php echo e($user->max_ram - $user->usedRam()); ?> MB</span>
                        <span class="bg-zinc-800 text-white text-xs font-mono py-1 px-2 rounded border border-white/5" id="ram-val">1024 MB</span>
                    </div>
                </div>
                <input type="range" name="memory" min="512" max="<?php echo e($user->max_ram - $user->usedRam()); ?>" step="512" value="1024" 
            ...
            <div class="group">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-zinc-500">Max: <?php echo e($user->max_cpu - $user->usedCpu()); ?>%</span>
                        <span class="bg-zinc-800 text-white text-xs font-mono py-1 px-2 rounded border border-white/5" id="cpu-val">100 %</span>
                    </div>
                </div>
                <input type="range" name="cpu" min="50" max="<?php echo e($user->max_cpu - $user->usedCpu()); ?>" step="50" value="100" 
                    class="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-indigo-500 hover:accent-indigo-400 transition-all"
            ...
            <div class="group">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-zinc-500">Max: <?php echo e($user->max_disk - $user->usedDisk()); ?> MB</span>
                        <span class="bg-zinc-800 text-white text-xs font-mono py-1 px-2 rounded border border-white/5" id="disk-val">5120 MB</span>
                    </div>
                </div>
                <input type="range" name="disk" min="1024" max="<?php echo e($user->max_disk - $user->usedDisk()); ?>" step="1024" value="5120" 
                    class="w-full h-1.5 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-indigo-500 hover:accent-indigo-400 transition-all"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\SSSSSS\stats\vortexdash\resources\views/servers/create.blade.php ENDPATH**/ ?>