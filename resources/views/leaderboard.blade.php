...
<div class="max-w-4xl mx-auto animate-fade">
    ...
    <div class="flex flex-col md:flex-row justify-center items-end gap-4 md:gap-6 mb-12 mt-8 px-4">
        ...
        <div class="w-full md:w-1/3 bg-zinc-900 border border-white/5 rounded-t-2xl rounded-b-lg p-6 text-center relative flex flex-col items-center order-2 md:order-1 h-[220px] justify-end">
            ...
            <div class="w-16 h-16 rounded-full bg-zinc-800 flex items-center justify-center text-xl font-bold text-zinc-400 mb-3">{{ substr($topUsers[1]->name, 0, 1) }}</div>
            <h3 class="text-white font-bold truncate w-full">{{ $topUsers[1]->name }}</h3>
            <p class="text-amber-500 font-mono text-sm mt-1">{{ number_format($topUsers[1]->coins) }} Coins</p>
        </div>
        @endif
        ...
        <div class="w-full md:w-1/3 bg-gradient-to-b from-amber-500/10 to-zinc-900 border border-amber-500/30 rounded-t-2xl rounded-b-lg p-6 text-center relative flex flex-col items-center order-1 md:order-2 h-[260px] justify-end shadow-[0_0_40px_rgba(245,158,11,0.1)]">
            ...
            <div class="w-20 h-20 rounded-full bg-zinc-950 border-2 border-amber-500/50 flex items-center justify-center text-2xl font-bold text-amber-500 mb-3 shadow-inner">{{ substr($topUsers[0]->name, 0, 1) }}</div>
            <h3 class="text-white font-bold text-lg truncate w-full">{{ $topUsers[0]->name }}</h3>
            <p class="text-amber-500 font-mono font-bold mt-1">{{ number_format($topUsers[0]->coins) }} Coins</p>
        </div>
        @endif
        ...
        <div class="w-full md:w-1/3 bg-zinc-900 border border-white/5 rounded-t-2xl rounded-b-lg p-6 text-center relative flex flex-col items-center order-3 h-[200px] justify-end">
            ...
            <div class="w-14 h-14 rounded-full bg-zinc-800 flex items-center justify-center text-lg font-bold text-zinc-400 mb-3">{{ substr($topUsers[2]->name, 0, 1) }}</div>
            <h3 class="text-white font-bold truncate w-full">{{ $topUsers[2]->name }}</h3>
            <p class="text-amber-500 font-mono text-sm mt-1">{{ number_format($topUsers[2]->coins) }} Coins</p>
        </div>
        @endif
    </div>
        ...
        <div class="divide-y divide-white/5">
            ...
            <div class="flex items-center justify-between p-4 px-6 hover:bg-white/[0.02] transition-colors">
                <div class="flex items-center gap-4">
                    ...
                    <span class="text-zinc-200 font-medium">{{ $u->name }}</span>
                </div>
                <div class="text-amber-500 font-mono text-sm">
                    {{ number_format($u->coins) }} <span class="text-zinc-600 text-xs">V-C</span>
                </div>
            </div>
            @endforeach
        </div>
