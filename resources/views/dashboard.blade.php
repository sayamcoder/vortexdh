...
<div id="app" class="space-y-10 animate-fade pb-20">
            ...
            <div class="bg-zinc-900 border border-white/5 rounded-[2rem] p-8 md:p-10 relative overflow-hidden shadow-2xl group">
                ...
                <div class="relative z-10">
                    <div class="flex flex-col md:flex-row md:items-center gap-6 mb-8">
                        ...
                        <div>
                            <h2 class="text-3xl font-bold text-white tracking-tight">Welcome, Commander {{ Auth::user()->name }}</h2>
                    ...
                    <div class="space-y-3 mb-8 max-w-lg">
                        <div class="flex justify-between items-end px-1">
                            ...
                            <span class="text-xs font-mono font-bold text-indigo-400">{{ $user->xp ?? 0 }} / {{ $user->xpForNextLevel() }} <span class="text-zinc-600">XP</span></span>
                        </div>
                        <div class="w-full bg-black/40 rounded-full h-2 border border-white/5 overflow-hidden relative shadow-inner">
                            <div class="h-full bg-gradient-to-r from-indigo-600 via-indigo-400 to-blue-400 rounded-full transition-all duration-1000 ease-out relative" style="width: {{ $user->xpProgress() }}%">
                    ...
                    <div class="flex flex-wrap gap-4">
                        <a href="{{ route('servers.create') }}" class="px-8 py-3 bg-white text-black text-xs font-black rounded-xl hover:bg-zinc-200 transition-all uppercase tracking-widest shadow-[0_10px_20px_rgba(255,255,255,0.1)] active:scale-95">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-4 bg-zinc-950 border border-white/5 rounded-2xl group hover:border-indigo-500/30 transition-all">
                        <div class="flex items-center gap-4">
                            <div class="w-2 h-2 rounded-full {{ $node['attributes']['maintenance_mode'] ? 'bg-amber-500' : 'bg-emerald-500' }} shadow-[0_0_8px_currentColor]"></div>
                            <div>
                                <p class="text-xs font-bold text-zinc-200">{{ $node['attributes']['name'] }}</p>
                                <p class="text-[9px] text-zinc-600 font-mono mt-0.5">{{ $node['attributes']['fqdn'] }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                             <span class="text-[10px] font-mono font-bold text-zinc-500">{{ rand(12, 35) }}ms</span>
                        </div>
                    </div>
                    @empty
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
        ...
        @endphp
        @foreach($matrix as $stat)
        <div class="bg-zinc-900/50 border border-white/5 rounded-3xl p-6 transition-all hover:bg-zinc-900 hover:border-{{ $stat['color'] }}-500/20 group shadow-sm">
            <div class="flex justify-between items-center mb-5">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-{{ $stat['color'] }}-500/10 flex items-center justify-center text-{{ $stat['color'] }}-400 border border-{{ $stat['color'] }}-500/20 group-hover:scale-110 transition-transform">
                    ...
                    <span class="text-[10px] font-black text-zinc-500 uppercase tracking-widest group-hover:text-zinc-300 transition-colors">{{ $stat['label'] }}</span>
                </div>
                @php $pct = min(100, ($stat['used'] / max(1, $stat['max'])) * 100); @endphp
                <span class="text-[10px] font-mono {{ $pct > 80 ? 'text-red-400' : 'text-zinc-500' }}">{{ round($pct) }}%</span>
            </div>
            
            <div class="text-2xl font-bold text-white mb-4 tracking-tight">
                {{ number_format($stat['used']) }} <span class="text-xs font-medium text-zinc-600">/ {{ number_format($stat['max']) }} {{ $stat['unit'] }}</span>
            </div>
            
            <div class="w-full bg-black/40 rounded-full h-1 overflow-hidden relative shadow-inner">
                <div class="h-full bg-{{ $stat['color'] }}-500 rounded-full shadow-[0_0_10px_currentColor] transition-all duration-1000 ease-out" style="width: {{ $pct }}%"></div>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div class="pt-6">
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
                ...
                <div class="px-2.5 py-1 rounded-md bg-zinc-900 border border-white/5 flex items-center shadow-inner">
                    ...
                    <span class="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{{ $servers->count() }} Instances</span>
                </div>
            </div>
            <div class="h-px flex-1 bg-gradient-to-r from-white/5 via-white/10 to-white/5 mx-8 hidden sm:block"></div>
            <a href="{{ route('servers.create') }}" class="group text-[11px] font-black text-indigo-400 hover:text-white transition-all uppercase tracking-widest flex items-center bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 px-5 py-2.5 rounded-xl shadow-lg shadow-indigo-600/5">
                New Signal
        ...
        @if($servers->isEmpty())
            <div class="bg-zinc-900/30 border border-zinc-800/50 border-dashed rounded-[3rem] py-24 text-center flex flex-col items-center group transition-all duration-500 hover:border-indigo-500/20">
                ...
                <a href="{{ route('servers.create') }}" class="px-10 py-4 bg-white hover:bg-zinc-200 text-zinc-900 text-xs font-black rounded-2xl transition-all uppercase tracking-widest shadow-2xl active:scale-95">
    ...
    <div class="bg-zinc-900 border border-white/5 rounded-[2rem] overflow-hidden shadow-2xl pt-2">
        <div class="px-8 py-5 border-b border-white/5 flex justify-between items-center bg-gradient-to-r from-zinc-900 to-zinc-900/50">
            ...
            <a href="{{ route('account.activity') }}" class="text-[10px] font-bold text-zinc-500 hover:text-indigo-400 transition-colors flex items-center gap-1.5 uppercase tracking-widest group">
                ...
                <div class="px-8 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-white/[0.01] transition-colors group">
                    <div class="flex items-center gap-6">
                        <div class="hidden sm:flex flex-shrink-0 w-10 h-10 items-center justify-center rounded-2xl bg-zinc-950 border border-white/5 text-zinc-500 group-hover:border-indigo-500/30 group-hover:bg-indigo-500/5 transition-all duration-500 shadow-inner">
                             @if($log->type == 'success') <svg class="w-5 h-5 text-emerald-500/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                             @elseif($log->type == 'warning') <svg class="w-5 h-5 text-amber-500/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                             @else <svg class="w-5 h-5 text-indigo-500/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> @endif
                        </div>
                        <div>
                            <div class="flex items-center gap-3 mb-1">
                                <p class="text-sm font-bold text-zinc-200 group-hover:text-white transition-colors">{{ $log->action }}</p>
                                <span class="px-1.5 py-0.5 rounded text-[8px] font-mono font-bold bg-zinc-900 text-zinc-600 group-hover:text-indigo-400 group-hover:border-indigo-500/20 transition-all border border-white/5">0x{{ strtoupper(dechex($log->id + 40000)) }}</span>
                            </div>
                            <p class="text-xs text-zinc-500 font-medium tracking-tight">{{ $log->description }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 sm:block sm:text-right">
                        <p class="text-[10px] text-zinc-400 font-mono uppercase tracking-widest">{{ $log->created_at->format('M d') }}</p>
                        <p class="text-[10px] text-zinc-600 font-mono uppercase tracking-widest mt-1 opacity-60">{{ $log->created_at->format('H:i:s') }}</p>
                    </div>
                </div>
            @empty
