...
<div id="app" class="space-y-10 animate-fade">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.users') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-zinc-300 bg-zinc-900 border border-white/10 rounded-lg hover:bg-zinc-800 hover:text-white transition-all">
            ...
            <a href="{{ route('admin.servers') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-zinc-300 bg-zinc-900 border border-white/10 rounded-lg hover:bg-zinc-800 hover:text-white transition-all">
                <svg class="w-4 h-4 mr-2 text-zinc-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"></path></svg>
                Fleet
            </a>
            <a href="{{ route('admin.store') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-zinc-300 bg-zinc-900 border border-white/10 rounded-lg hover:bg-zinc-800 hover:text-white transition-all">
        ...
        <div class="bg-zinc-900 border border-white/5 rounded-xl p-5 hover:border-white/10 transition-colors">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-2xl font-semibold text-white mt-1">{{ number_format($stats['total_users']) }}</h3>
                </div>
        ...
        <div class="bg-zinc-900 border border-white/5 rounded-xl p-5 hover:border-white/10 transition-colors">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-2xl font-semibold text-white mt-1">{{ number_format($stats['total_servers']) }}</h3>
                </div>
        ...
        <div class="bg-zinc-900 border border-white/5 rounded-xl p-5 hover:border-white/10 transition-colors">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-2xl font-semibold text-white mt-1">{{ number_format($stats['total_coins']) }}</h3>
                </div>
        ...
        <div class="bg-zinc-900 border border-white/5 rounded-xl p-5 hover:border-white/10 transition-colors">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-2xl font-semibold text-white mt-1">{{ number_format($stats['total_ram'] / 1024, 1) }} GB</h3>
                </div>
                ...
                <div class="divide-y divide-white/5">
                    ...
                    <div class="p-4 hover:bg-white/5 transition-colors flex items-start gap-3">
                        ...
                        <div class="mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 
                            @if($log->type == 'success') bg-emerald-500
                            @elseif($log->type == 'warning') bg-amber-500
                            @elseif($log->type == 'danger') bg-red-500
                            @else bg-blue-500 @endif">
                        </div>
                        
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-zinc-200 truncate">{{ $log->action }}</p>
                            <p class="text-xs text-zinc-500 mt-0.5 line-clamp-2">{{ $log->description }}</p>
                            ...
                            <div class="flex items-center mt-2 justify-between">
                                 @if($log->user)
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-[10px] text-zinc-400">{{ $log->user->name }}</span>
                                    </div>
                                 @else
                                    <span class="text-[10px] text-zinc-500 font-mono uppercase">SYSTEM</span>
                                 @endif
                                 <span class="text-[10px] text-zinc-600">{{ $log->created_at->diffForHumans(null, true) }}</span>
                            </div>
                        </div>
                    </div>
                    @empty
