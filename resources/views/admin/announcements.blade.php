@extends('layouts.app')
@section('title', 'Broadcast System')
...
<div class="animate-fade grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1">
        ...
        <form action="{{ route('admin.announcements.create') }}" method="POST" class="bg-zinc-900 border border-white/5 rounded-xl p-6 shadow-sm">
            @csrf
            ...
            <div class="bg-zinc-900 border border-white/5 rounded-xl p-5 flex items-start justify-between group">
                <div class="flex items-start gap-4">
                    <div class="mt-1 w-2 h-2 rounded-full flex-shrink-0 
                        @if($announcement->type == 'success') bg-emerald-500 shadow-[0_0_8px_#10B981]
                        @elseif($announcement->type == 'warning') bg-amber-500 shadow-[0_0_8px_#F59E0B]
                        @elseif($announcement->type == 'danger') bg-red-500 shadow-[0_0_8px_#EF4444]
                        @else bg-blue-500 shadow-[0_0_8px_#3B82F6] @endif">
                    </div>
                    <div>
                        <h3 class="text-sm font-bold text-white">{{ $announcement->title }}</h3>
                        <p class="text-sm text-zinc-400 mt-1">{{ $announcement->message }}</p>
                        <p class="text-[10px] text-zinc-600 mt-2">{{ $announcement->created_at->diffForHumans() }}</p>
                    </div>
                </div>
                
                <form action="{{ route('admin.announcements.delete', $announcement) }}" method="POST">
                    @csrf
                    @method('DELETE')
